...
/** The reason a `logWaveMutation` failure carries: the writer reports to stderr, not to us. */
const WAVE_APPEND_FAILED = 'the wave-action or its paired cli-exit did not land'
...
function adoptPlan(ctx, manifest, plan) {
  ...
  if (!ok) {
    ...
    return trajectoryHaltStep('wave-state create', WAVE_APPEND_FAILED)
  }
...
export function runRecordBatch(ctx, { results }) {
  ...
  if (!logOk) {
    ...
    return finishStep(
      ctx,
      manifest,
      trajectoryHaltStep('wave-state record-batch', WAVE_APPEND_FAILED, { banners })
    )
  }
  ...
  if (context === 'inter-wave') {
    ...
    if (!ok) {
      ...
      return finishStep(
        ctx,
        manifest,
        trajectoryHaltStep('wave-state record-verify', WAVE_APPEND_FAILED, { banners })
      )
    }
    return finishStep(ctx, manifest, decorate(ctx, nextRaw, manifest, { banners }))
  }
...
export function runReplan(ctx, { results }) {
  ...
  if (!ok) {
    ...
    return finishStep(ctx, manifest, trajectoryHaltStep('wave-state replan', WAVE_APPEND_FAILED))
  }
