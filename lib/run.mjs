...
import { inspectChange, planCoverage, tickTasks } from './artifacts.mjs'
import { computeFingerprint, resolvePlanReuse, writeFingerprint } from './plan-fingerprint.mjs'
...
import { planVerification, judgeVerification } from './verify.mjs'
 ...
 *
 * @param {object} manifest
 * @returns {{spend: Array<{wave: number, outputTokens: number|null}>, outputTokens: number|null|undefined}}
 */
export function summarizeUsage(manifest) {
  const entries = Array.isArray(manifest && manifest.usage) ? manifest.usage : []
  if (!entries.length) return { spend: [], outputTokens: undefined }

  const byWave = new Map()
  for (const entry of entries) {
    if (!Number.isInteger(entry && entry.wave)) continue
    const current = byWave.has(entry.wave) ? byWave.get(entry.wave) : 0
    byWave.set(
      entry.wave,
      current === null || entry.outputTokens === null ? null : current + entry.outputTokens
    )
  }
  ...
  return {
    spend: [...byWave.entries()]
      .sort((a, b) => a[0] - b[0])
      .map(([wave, outputTokens]) => ({ wave, outputTokens })),
...
function verifyStep(ctx, manifest, { context, label, changed, carried, action = 'verify' }) {
  ...
  if (!plan.steps.length) {
    const reason = (plan.skipped[0] && plan.skipped[0].reason) || 'no detectable commands'
    return makeStep({
      action,
      ...carried,
      context,
...
export function runRecordBatch(ctx, { results }) {
  ...
  if (isolate && laneFoldCandidates.length) {
    const base = typeof pending.mergeBase === 'string' && pending.mergeBase ? pending.mergeBase : headCommit(ctx)
...
export function runJudge(ctx, { context, results }) {
  ...
  // The skip is recorded as the run's own, with the reason the planner gave.
  const skipped = !plan || !Array.isArray(plan.steps) || !plan.steps.length
  const reason = skipped
    ? (plan && plan.skipped && plan.skipped[0] && plan.skipped[0].reason) || 'no detectable commands'
    : null
 ...
 *
 * @returns {{action: 'complete'|'halt', exitCode: number, summary: string, banners: string[]}}
 */
export function runClose(ctx, { results = [], halt = null, hostBanners = [], hostObserved = null } = {}) {
  const { root } = ctx
  const manifest = readManifest(root)
  if (!manifest) {
    ...
    return {
      ...
      summary:
        `SHIP HALTED — ${reason || 'the run ended before it started'}\n\n` +
