...
/**
 * Split a group whose tasks claim overlapping paths.
 *
 * Tasks in a group run in parallel **in one working tree**. Their independence
 * has always been asserted by the classifier and checked by nothing, which made
 * it the last correctness-critical judgement in this flow living in prose. Two
 * agents editing one file concurrently is a lost write, and nothing downstream
 * would notice.
 *
 * Re-group rather than halt: the planner already clamps an over-eager `opus`
 * instead of failing the run, and the same reasoning applies here. A collision
 * is a scheduling problem with an obvious fix, not a malformed input.
 *
 ...
 * close it. The value is that the assumption is now stated and checked instead
 * of merely assumed — an unpredicted collision is a wrong prediction, which is
 * a thing that can be improved, rather than a silent property of the design.
 *
 * @returns {{groups: Map<number, object[]>, moved: object[]}}
 */
function resolvePathCollisions(implTasks) {
  const moved = []
  ...
  const ordered = [...byGroup.keys()].sort((a, b) => a - b)
  let overflow = (ordered.length ? Math.max(...ordered) : 0) + 1

  for (const group of ordered) {
    // Stable order so the same input always moves the same task: the first
    // claimant of a path keeps its slot, later ids yield.
    const members = byGroup.get(group).slice().sort((a, b) => String(a.id).localeCompare(String(b.id)))
    const claimed = new Map() // path -> id that holds it
    const keep = []
    const evicted = []
    const records = []
    ...
    for (const t of members) {
      ...
      if (clash) {
        evicted.push(t)
        records.push({ id: t.id, from: group, path: clash, conflictsWith: claimed.get(clash) })
        continue
      }
      for (const p of paths) claimed.set(p, t.id)
      keep.push(t)
    }

    byGroup.set(group, keep)
    // Evicted tasks land in one trailing group rather than one group each: they
    // collided with a task in the original group, not necessarily each other.
    // Any collision among themselves is caught when that group is processed —
    // `ordered` is appended to during iteration, so the new group is visited.
    if (evicted.length) {
      for (const t of evicted) t.group = overflow
      for (const r of records) moved.push({ ...r, to: overflow })
      byGroup.set(overflow, evicted)
      ordered.push(overflow)
      overflow += 1
    }
  }

  return { groups: byGroup, moved }
}
...
export function planWaves(input, opts = {}) {
  ...
  // Independence is a claim, so check it before scheduling on it: two tasks in
  // one group that name the same file cannot run in parallel in one tree.
  const { groups: byGroup, moved: regrouped } = resolvePathCollisions(implTasks)
  ...
  for (const m of regrouped) {
    warnings.push(
      `task ${m.id} moved out of wave ${m.from}: it claims ${m.path}, already claimed by ${m.conflictsWith}`
    )
  }
...
export function formatPlan(plan) {
  ...
  for (const m of plan.regrouped || []) {
    lines.push(`  regrouped ${m.id}: wave ${m.from} → ${m.to ?? 'later'} (${m.path} held by ${m.conflictsWith})`)
  }
