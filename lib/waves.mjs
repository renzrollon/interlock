...
/**
 * Skip reason for the leading failing-test wave of a TDD change.
 *
 * The suite is SUPPOSED to be failing at that point, so the ordinary check
 * would spend `LIMITS.interWaveFixAttempts` trying to repair tests that were
 * just written on purpose, and then halt the run. The next wave's check is the
 * real gate: by then the implementation exists, and a still-red suite is a
 * genuine failure that correctly burns those attempts.
 */
export const SKIP_VERIFY_RED = 'red-wave-expected-failing'
...
// Per-task shape only. The dependency check is a SEPARATE call because it needs
// the resolved red wave, and the red wave can only be resolved once the tasks
// are known to be well-formed — so the two cannot happen in one pass.
 ...
 *     The ONE exception is a test task in the red wave of a TDD change: that
 *     one is not deferred, it runs first, and "make the failing suite pass" is
 *     exactly the dependency an author of such a change means.
 *   - A RED-WAVE task depending on anything outside the red wave. This
 *     rejection exists only because the exception above does: a deferred test
 *     task ran last, so any edge it declared was satisfiable and none was
 *     checked. A red-wave task runs FIRST, so the same edge now names work
 *     that has not happened yet.
 *
 * @param {Array} tasks
 * @param {number|null} redWave the group that runs as the leading failing-test
 *   wave, or null. Null is the whole of today's behaviour: `t.group === null`
 *   is never true for a validated task, so every branch below collapses to the
 *   rule it had before this parameter existed.
 */
function validateDependencies(tasks, redWave = null) {
  const byId = new Map(tasks.map(t => [t.id, t]))
  const inRedWave = task => redWave !== null && task.group === redWave
  ...
  for (const t of tasks) {
    for (const dep of dependsOnIds(t)) {
      ...
      if (!t.isTestTask && target.isTestTask && !inRedWave(target)) {
        fail(
          `task ${t.id} depends on test task ${dep}, which runs in the trailing test wave — ` +
            `a dependency can only name work that runs earlier`
        )
      }
      // The red wave runs first, so nothing it declares can point outward.
      // Unchecked before this existed and correct then: a deferred test task
      // ran after everything, so any edge it named had already happened.
      if (inRedWave(t) && !inRedWave(target)) {
        fail(
          `task ${t.id} runs in the red wave (section ${redWave}) and depends on ${dep}, which ` +
            `runs after it — a leading failing-test wave cannot depend on the work it pins`
        )
      }
      // No backward-edge check is needed for a red-wave TARGET, and that is a
      // consequence rather than an omission: `resolvePlanRedWave` only honours
      // a claim on the LOWEST group, so a red target is always in the first
      // section and no task can sit earlier than it. If that invariant is ever
      // relaxed, this is the check that has to come back.
...
function packLanes(entries, maxParallel) {
  ...
  // The lanes are already built, so each one's path set is immutable for the
  // life of this call. Computed once here rather than inside the loop: a lane
  // deferred across k batches would otherwise be re-walked k times.
  const pathSets = new Map(entries.map(e => [e, laneCanonicalPaths(e.tasks)]))
  const batches = []
  while (remaining.length) {
    ...
    for (const entry of remaining) {
      const paths = pathSets.get(entry)
      let clash = false
...
function foldSingletonWaves(planned) {
  ...
  for (const wave of planned) {
    ...
    // Never fold NON-RED work onto the red wave. Folding is "ordering, not a
    // checkpoint", which is true between two implementation waves and false
    // here: the red wave's whole content is a suite that is supposed to be
    // failing, so absorbing the work that makes it pass into the same wave
    // would erase the boundary the shape exists to draw — and hand the merged
    // wave the red wave's skipped verification. A red layer folding onto an
    // earlier red layer is still just ordering, and still folds.
    const ontoRed = previous && previous.red === true && wave.red !== true
    if (wave.taskCount === 1 && previous && !ontoRed) {
...
/**
 * Adjudicate the caller's red-wave claim against the classified tasks.
 *
 * Two ways a claim is refused, both structural and both checkable here without
 * knowing anything about where the claim came from:
 *
 *   - The group holds no test task. Then there is no failing suite to run
 *     first, and honouring the claim would only skip that wave's verification.
 *   - The group is not the lowest one present. A failing-test section that runs
 *     after implementation is not a red wave; treating it as one would move
 *     tests earlier than the author put them, in a change they did not write
 *     for TDD.
 *
 * A non-integer is not a refusal, it is the absence of a claim — that is how
 * every non-TDD run arrives here.
 *
 * @returns {{group: number|null, refused: string|null}}
 */
function resolvePlanRedWave(requested, tasks) {
  if (!Number.isInteger(requested)) return { group: null, refused: null }
  const inGroup = tasks.filter(t => t.group === requested)
  if (!inGroup.length) {
    return { group: null, refused: `no task is classified in section ${requested}` }
  }
  if (!inGroup.some(t => t.isTestTask)) {
    return {
      group: null,
      refused: `section ${requested} holds no test task, so there is no failing suite to run first`
    }
  }
  const lowest = Math.min(...tasks.map(t => t.group))
  if (requested !== lowest) {
    return {
      group: null,
      refused:
        `section ${requested} is not the first section (section ${lowest} runs earlier), and a ` +
        `failing-test section that runs after implementation is not a red wave`
    }
  }
  return { group: requested, refused: null }
}
 ...
 *
 *   `redWave` is the group that runs as a LEADING failing-test wave instead of
 *   being deferred — the execution half of the TDD task shape. It is resolved by
 *   the caller (`resolveRedWave` in `lib/run.mjs`) from the `tasks.md` marker or
 *   an explicit flag, never inferred here: this module is pure and cannot read
 *   the file the shape was written in. Absent — which is every change that is
 *   not authored for TDD — every branch it gates collapses to the behaviour this
 *   planner had before it existed.
 */
export function planWaves(input, opts = {}) {
  ...
  // Refused rather than honoured when it names nothing runnable: a red wave is
  // a claim about the task list, and a claim the task list contradicts must not
  // reorder the run. Refusing is a null here plus a warning below — never a
  // halt, because the change is still perfectly implementable in the ordinary
  // shape, and never silence, because a shape the operator asked for and did
  // not get is exactly the thing that has to be said out loud.
  const redWave = resolvePlanRedWave(opts.redWave, tasks)
  validateDependencies(tasks, redWave.group)
  ...
  // The red wave's test tasks are planned as an ordinary implementation wave —
  // they keep their group, and therefore their lanes, batches, cohesion folds
  // and model clamp, all unchanged. Everything else defers as it always has.
  //
  // With no red wave `t.group === null` is never true for a validated task, so
  // this is the same partition it was before, expression for expression.
  const implTasks = tasks.filter(t => !t.isTestTask || t.group === redWave.group)
  const testTasks = tasks.filter(t => t.isTestTask && t.group !== redWave.group)
  ...
  if (solo) {
    ...
    for (const group of groups) {
      ...
      layers.forEach((layer, depth) => {
        ...
        // `red` is spread in only when this IS the red wave, so a plan for an
        // ordinary change carries no new key and stays byte-comparable with the
        // plans every existing reader and fixture was written against.
        planned.push({
          group,
          taskCount: layerTasks.length,
          batches,
          ...(group === redWave.group ? { red: true } : {})
        })
      })
    }
  ...
  // Both directions are spoken. A refused claim is the operator not getting the
  // shape they asked for; an honoured one changes what the run verifies and
  // when, which nobody should have to infer from a missing checkpoint.
  if (redWave.refused) {
    warnings.push(`red wave refused: ${redWave.refused} — planned in the ordinary shape instead`)
  }
  if (redWave.group !== null) {
    warnings.push(
      `section ${redWave.group} is the red wave: its failing tests run FIRST rather than ` +
        `deferring to the trailing test wave, and its inter-wave check is skipped because the ` +
        `suite is expected to fail there — so this run cannot tell a genuinely failing suite ` +
        `from one that already passed. The next wave's check is the gate.`
    )
  }
  ...
  return {
    ...
    // The resolved group, not the requested one: a refused claim reads as no
    // red wave here, and the refusal is in `warnings` where a reader looks for
    // what the planner decided against.
    redWave: redWave.group,
    implCount,
...
export function formatPlan(plan) {
  ...
  for (const wave of plan.waves) {
    ...
    // Named on the wave's own line rather than only in the warnings, because
    // this is the one wave whose failing check is not a problem — an operator
    // watching the run has to be able to see which one that is.
    const redNote = wave.red === true ? ' — RED: failing tests, check skipped' : ''
    lines.push(`  Wave ${wave.group}: ${wave.taskCount} task(s)${batchNote}${redNote}`)
...
function makeWave(kind, group, tasks, maxParallel, laneCaps, opts = {}) {
  const serialized = []
  ...
  return {
    kind,
    group,
    // Preserved across a replan. The marker is what stops the red wave's
    // failing suite being treated as a defect, so a revision that rebuilt the
    // group without it would restore exactly the halt this wave exists to
    // avoid — and would do it only on replanned runs, which is the hardest
    // version of that bug to find.
...
function adoptWave(wave, kind, index, maxParallel, warnings) {
  ...
  return {
    kind,
    group,
    // Carried from the plan rather than re-derived: whether a wave is the red
    // one is a fact about how it was PLANNED, and the run state is the only
    // thing the engine sees afterwards. Spread in only when true, so a state
    // for an ordinary change is unchanged — and a state written before red
    // waves existed reads as `undefined`, which every check below treats as
    // not-red rather than as an error.
  ...
  if (next.cursor.batchIndex >= wave.batches.length) {
    if (next.waves[next.cursor.waveIndex + 1]) {
      ...
      if (isDocsOnlyWave(wave)) {
        ...
        advanceWave(next)
      } else if (wave.red === true) {
        // Ahead of the cap check on purpose: the red wave must not consume one
        // of the run's inter-wave verification slots. Checking it would mean
        // spending `LIMITS.interWaveFixAttempts` on a suite that is failing
        // because it was asked to, and then halting the run over it.
        next.skippedVerifications.push({
          wave: wave.group,
          waveKind: wave.kind,
          reason: SKIP_VERIFY_RED
        })
        advanceWave(next)
...
export function applyReplan(state, revisedGroups) {
  ...
  for (const revision of revisedGroups) {
    ...
    if (at >= 0) {
      ...
      // Read off the wave being replaced, not recomputed: the red wave is a
      // decision the PLAN made, and a replan revises a group's tasks, never
      // which group leads with the failing suite.
      const wasRed = next.waves[at] && next.waves[at].red === true
