...
//      run in parallel; `LIMITS.maxTasksPerAgent` bounds how much one
//      interruption can discard, and a cap of 1 reproduces one agent per task.
//
...
import { LIMITS, EFFORT, clampParallel } from './limits.mjs'
 ...
 * close it.
 */
function buildLanes(tasks, serialized, group, maxTasksPerAgent) {
  ...
  const cap = Number.isInteger(maxTasksPerAgent) && maxTasksPerAgent > 0 ? maxTasksPerAgent : 1
  ...
  const lanes = []
  for (const component of ranked) lanes.push(...chunk(component, cap))
  return lanes
}
 ...
 */
function packLanes(lanes, maxParallel) {
  const remaining = lanes.slice()
  const batches = []
  while (remaining.length) {
    ...
    for (const lane of remaining) {
      const paths = laneCanonicalPaths(lane)
      let clash = false
...
/**
 * A group's tasks as batches of lanes.
 *
 ...
 * holds exactly one task, which is what a cap of 1 reproduces exactly.
 */
function collisionAwareChunk(tasks, maxParallel, serialized, group, maxTasksPerAgent) {
  return packLanes(buildLanes(tasks, serialized, group, maxTasksPerAgent), maxParallel)
}
 ...
 * @param {{maxParallel?: number, maxTasksPerAgent?: number}} opts
 *   `maxTasksPerAgent` overrides the lane cap for this plan only; the default is
 *   and stays `LIMITS.maxTasksPerAgent`, so the cap is still stated once. It is
 *   an override because 1 is the documented rollback lever — it reproduces one
 *   agent per task exactly — and a lever nothing can pull is not a lever.
 */
export function planWaves(input, opts = {}) {
  ...
  const maxTasksPerAgent =
    Number.isInteger(opts.maxTasksPerAgent) && opts.maxTasksPerAgent > 0
      ? opts.maxTasksPerAgent
  ...
  for (const group of groups) {
    ...
    layers.forEach((layer, depth) => {
      ...
      planned.push({
        ...
        batches: collisionAwareChunk(layerTasks, maxParallel, serialized, group, maxTasksPerAgent)
      })
    })
  }
  ...
  const widest = planned.reduce((m, w) => Math.max(m, w.taskCount), 0)

  const { waves, folded } = foldSingletonWaves(planned)
  ...
  // The test wave is chunked by task, not by lane: test tasks are deferred to a
  // trailing wave precisely because they are not the sequential same-file work
  // lanes exist to fold, and a lane there would only reduce the parallelism the
  // deferral bought.
  ...
  const testWave = testTasks.length
    ? {
        ...
        batches: layerByDependencies(testTasks).flatMap(layer =>
          chunk(layer, maxParallel).map(b => b.map(t => [t]))
        )
      }
    : null
  ...
  for (const wave of waves) {
    for (const lane of waveLanes(wave)) {
  ...
  for (const lane of lanes) {
    warnings.push(
      `tasks ${lane.ids.join(', ')} share a file in wave ${lane.group}; folded into one lane ` +
        `run by a single tier-${lane.tier} agent in that order`
    )
  }
  ...
  for (const m of serialized) {
    warnings.push(
      `task ${m.id} serialized in wave ${m.group}: it claims ${m.path}, already claimed by ` +
  ...
  // reading the plan has to be able to see it took effect.
  for (const d of deferred) {
    warnings.push(
      `task ${d.id} declares dependsOn ${d.after.join(', ')}; it runs after them in wave ` +
        `${d.group}, in a later batch or wave rather than beside them`
    )
  }
...
export function formatPlan(plan) {
  ...
  for (const m of plan.serialized || []) {
    lines.push(
      `  serialized ${m.id}: same lane in wave ${m.group} (${m.path} held by ${m.conflictsWith})`
    )
  }
  for (const d of plan.deferred || []) {
    lines.push(`  ordered ${d.id}: after ${d.after.join(', ')} in wave ${d.group} (dependsOn)`)
  }
  ...
  // from an agent count that is smaller than the task count.
  for (const lane of plan.lanes || []) {
    lines.push(
      `  lane ${lane.ids.join(' → ')}: ${lane.ids.length} tasks in wave ${lane.group} on one ` +
...
function makeWave(kind, group, tasks, maxParallel, maxTasksPerAgent) {
  const serialized = []
  const cap =
    Number.isInteger(maxTasksPerAgent) && maxTasksPerAgent > 0
      ? maxTasksPerAgent
      : LIMITS.maxTasksPerAgent
  return {
    kind,
    ...
    // A replanned group is laned under the SAME cap the rest of the run was
    // planned with, read off the state. Falling back to the current LIMITS value
    // would let a replan mid-run produce lanes of a different length than the
    // plan the run started from.
    //
    ...
    batches: layerByDependencies(tasks).flatMap(layer =>
      collisionAwareChunk(layer, maxParallel, serialized, group, cap)
    )
  }
}
...
export function createRunState(plan, opts = {}) {
  ...
  return finalize({
    ...
    maxParallel,
    // Carried so a replan lanes its revision the way the plan was laned.
    maxTasksPerAgent:
      Number.isInteger(plan.maxTasksPerAgent) && plan.maxTasksPerAgent > 0
        ? plan.maxTasksPerAgent
        : LIMITS.maxTasksPerAgent,
...
export function applyReplan(state, revisedGroups) {
  ...
  for (const revision of revisedGroups) {
    ...
    if (at >= 0) {
      ...
      if (!revision.tasks.length) next.waves.splice(at, spans)
        ...
        next.waves.splice(
          at,
          spans,
          makeWave('impl', revision.group, revision.tasks, next.maxParallel, next.maxTasksPerAgent)
        )
      }
      continue
    }
    ...
    next.waves.splice(insertAt, 0, makeWave('impl', revision.group, revision.tasks, next.maxParallel, next.maxTasksPerAgent))
  }
