...
//
//   4. Lanes. A batch holds LANES, not tasks: a lane is an ordered task list
...
export function planWaves(input, opts = {}) {
  ...
  const planned = groups.map(group => {
    ...
    return {
      ...
      batches: collisionAwareChunk(groupTasks, maxParallel, serialized, group, maxTasksPerAgent)
    }
  })
  ...
  const testWave = testTasks.length
    ? { taskCount: testTasks.length, batches: chunk(testTasks, maxParallel).map(b => b.map(t => [t])) }
    : null
...
function makeWave(kind, group, tasks, maxParallel, maxTasksPerAgent) {
  ...
  return {
    ...
    batches: collisionAwareChunk(tasks, maxParallel, serialized, group, cap)
  }
}
...
export function applyReplan(state, revisedGroups) {
  ...
  for (const revision of revisedGroups) {
    ...
    if (at >= 0) {
      ...
      else next.waves[at] = makeWave('impl', revision.group, revision.tasks, next.maxParallel, next.maxTasksPerAgent)
      continue
    }
