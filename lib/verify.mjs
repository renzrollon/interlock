...
import { isDocsPath } from './risk.mjs'
...
export const SKIP_REASONS = {
  ...
  /** `--changed` named only documentation paths. */
...
function validateProfile(profile) {
  ...
  for (const section of ['unit', 'e2e', 'coverage', 'typecheck', 'lint']) {
 ...
 *
 * Every command comes from the profile or the caller **verbatim** — this
 * function never synthesizes one. `typecheck` and `lint` have their own
 * TEST-PROFILE.md sections, discovered once by `/interlock:fix-tests` the same
 * way `unit` is, and a caller may override either through `opts`. When neither
 * supplies one the step is skipped with a reason rather than guessed at: there
 * is no third branch where a command gets invented.
 *
 ...
 *   context?: string,        // inter-wave | final — inter-wave omits e2e/coverage by default
 *   changed?: string[]       // repo-relative paths the last wave touched
 * }} [opts]
...
export function planVerification(profile, opts = {}) {
  ...
  if (opts.context !== undefined && opts.context !== null) {
    if (!Object.prototype.hasOwnProperty.call(VERIFY_CONTEXTS, opts.context)) {
      fail(
        `verify context must be one of ${Object.keys(VERIFY_CONTEXTS).join('|')} (got "${opts.context}")`
      )
    }
  }

  const interWave = opts.context === 'inter-wave'
  const wantE2e = opts.e2e === true
  const wantCoverage = interWave ? opts.coverage === true : opts.coverage !== false
  ...
  const changed = Array.isArray(opts.changed)
    ? opts.changed.filter(p => typeof p === 'string' && p.trim())
    : null
  if (changed && changed.length && changed.every(p => isDocsPath(p))) {
    for (const kind of VERIFY_KINDS) {
      skip(kind, SKIP_REASONS.DOCS_ONLY, `${changed.length} changed path(s) are documentation`)
    }
    const banners = skipped.map(s => BANNERS.skipped(s.reason))
    if (!hasProfile) banners.unshift(BANNERS.noProfile)
  ...
  // --- typecheck: profile- or caller-supplied, and the one thing that survives
  // the budget.
  ...
  if (typecheckCommand) {
    ...
    skip('typecheck', SKIP_REASONS.NO_COMMAND, 'no typecheck command in the profile or from the caller')
  }
  ...
  if (!wantLint) skip('lint', SKIP_REASONS.CALLER_OPTED_OUT)
    ...
    skip('lint', SKIP_REASONS.NO_COMMAND, 'no lint command in the profile or from the caller')
  } else {
...
// --- oversized-result anti-swallow (add-ship-run-inspectability §3.3) ------
//
// This is deliberately NOT inside `judgeVerification` (design.md decision 3):
// the pure judge decides what a result *means*, this decides whether the
// result is even shaped the way spill was supposed to guarantee. A verify
// agent that pastes a spilled suite log into `cliStdout`, `detail`, or a
// `failures[]` entry instead of using the locator+preview `lib/spill.mjs`
// handed it has defeated the whole point of spilling; this is the check that
// catches it before the bytes reach a judgement (or a trajectory line).
//
// Fields checked are exactly the ones an agent could plausibly stuff a full
// suite log into: the free-text result fields plus every failure's text.
// `preview` itself is exempt — it is *allowed* to be long, that is what it is.
// Every field of a result that can carry free text. An incomplete list is an
// anti-swallow check with unguarded doors: `summary`, `message`, `error` and
// `notes` were all absent, so a whole red suite pasted into any of them passed
// straight through to judgement.
...
/**
 * Check that no verify result field exceeds the preview budget. Call this
 * before `judgeVerification` / `judgeUnitResult`, never after — an oversized
 * field is the leak this check exists to catch, not a data point to judge.
 *
 * @param {Array<object>} results the same array `judgeVerification` takes
 * @param {{maxChars?: number}} [opts] defaults to `LIMITS.verifyPreviewChars`
 * @returns {{ok: boolean, violations: Array<{kind: string, field: string, length: number, limit: number}>}}
 */
export function checkResultFieldSizes(results, opts = {}) {
  if (!Array.isArray(results)) fail('verification results must be an array')
  const limit = Number.isInteger(opts.maxChars) && opts.maxChars > 0 ? opts.maxChars : LIMITS.verifyPreviewChars

  const violations = []
  for (const r of results) {
    if (!isObject(r)) continue
    const kind = trimmedString(r.kind) || '(unknown kind)'

    for (const field of RESULT_TEXT_FIELDS) {
      const value = r[field]
      if (typeof value === 'string' && value.length > limit) {
        violations.push({ kind, field, length: value.length, limit })
      }
    }

    if (Array.isArray(r.failures)) {
      r.failures.forEach((failure, i) => {
        const text = failureText(failure)
        if (text.length > limit) {
          violations.push({ kind, field: `failures[${i}]`, length: text.length, limit })
        }
      })
    }
  }
