...
//
// The declaration is the load-bearing half. Every adapter states the same seven
...
//   cacheAccounting does it decompose that usage into cache reads and writes
//   billing         which account the run spends against
//
// `cacheAccounting` is separate from `usage` and not implied by it: the Workflow
// runtime demonstrates the gap by exposing a single cumulative spend scalar with
// no decomposition at all, and a run cannot banner a capability nobody declared.
//
...
export const CAPABILITY_VALUES = Object.freeze({
  ...
  cacheAccounting: Object.freeze([true, false]),
...
export const HOSTS = Object.freeze({
  claude: {
    ...
    capabilities: Object.freeze({
      ...
      hooks: true,
      usage: true,
      // The vendor envelope carries `cache_read_input_tokens` and a
      // `cache_creation` block split by lifetime tier; `readClaudeEnvelope`
      // keeps both.
  ...
  acp: {
    ...
    capabilities: Object.freeze({
      ...
      hooks: false,
      usage: false,
      // No usage envelope to decompose. Refined at creation only where `usage`
      // is, and it is not — an arbitrary ACP agent reports neither.
  ...
  codex: {
    ...
    capabilities: Object.freeze({
      ...
      hooks: false,
      usage: true,
      // Token usage without a cache decomposition — exactly the gap this
      // capability exists to make declarable rather than inferrable.
      cacheAccounting: false,
