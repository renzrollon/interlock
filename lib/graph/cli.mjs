...
function usage() {
  return `interlock-graph — lightweight codebase knowledge graph for agents

Usage:
  interlock-graph build [root]              Build graph into <root>/.claude/graph/
  interlock-graph update [root]             Incremental rebuild when files change
  interlock-graph report [root]             Print / regenerate GRAPH_REPORT.md
  interlock-graph query <q> [options]       Neighborhood query (BFS default)
  interlock-graph consumers <symbol|file>   Reverse imports / references
  interlock-graph path <A> <B>              Shortest path
  interlock-graph explain <node>            Node + neighbors
  interlock-graph docs <q> [options]        Ranked markdown section excerpts
  interlock-graph context <q> [options]     Graph subgraph + doc excerpts (default retrieval)
  interlock-graph docs-index [root]         Deterministic docs/ + openspec/specs TOC → docs-index.json
  interlock-graph which                     Print CLI path (for skills)
...
Examples:
  ...
  interlock-graph query AuthService --budget 1500
  interlock-graph consumers normalizeEmail
  interlock-graph path lib/auth app/api
  interlock-graph docs "authentication session" --budget 800
  interlock-graph context "auth login" --changed lib/auth.ts,app/api/login.ts --budget 2000
...
function main() {
  ...
  try {
        ...
        if (result.stats.files === 0) {
          process.stdout.write(
            '\nNo source files were indexed. Structural indexing covers ' +
              ...
              'Use `interlock-graph docs` and `interlock-graph docs-index` for ' +
    ...
    process.stderr.write(`interlock-graph: ${err.message || err}\n`);
    process.exit(1);
  }
}
