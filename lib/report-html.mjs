...
function indicatorsSection(indicators) {
  ...
  rows.push(
    row(
      ...
      'Whether the paths a run committed are the paths its plan named.',
      { headline: NOT_COMPUTABLE, basis: 'no corpus records the comparison' },
      html`<p class="finding">${i.planFidelity.diffMatchesPlan.reason}</p>` +
        html`<div class="requires"><span class="requires-label">Would require</span><span>${i.planFidelity.diffMatchesPlan.wouldRequire}</span></div>`
    )
  )
