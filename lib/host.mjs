...
/**
 * Plugin agent ids and tool allowlists for a slim spawn prefix.
 *
 * Claude Code's workflow `agent()` inherits the parent tool catalog and Skill
 * listing unless the call names a type and an allowlist. `workflows/ship.js`
 * cannot import this module (the runtime rejects `import()`), so it restates
 * the same four literals. `test/spine/plugin-agents.test.mjs` fails if they
 * drift. `kind === 'ping'` is the mechanical CLI path; everything else is a
 * worker.
 */
export const PING_AGENT = 'interlock:ping'
export const WORKER_AGENT = 'interlock:worker'
export const PING_TOOLS = Object.freeze(['Bash', 'Read', 'Write'])
export const WORKER_TOOLS = Object.freeze(['Read', 'Write', 'Edit', 'Grep', 'Glob', 'Bash'])

/**
 * @param {string} [kind]
 * @returns {{ type: string, tools: string[] }}
 */
export function spawnPrefix(kind) {
  return kind === 'ping'
    ? { type: PING_AGENT, tools: [...PING_TOOLS] }
    : { type: WORKER_AGENT, tools: [...WORKER_TOOLS] }
}
 ...
 * @property {string} [type]     plugin agent id (`interlock:ping` / `interlock:worker`)
 * @property {string[]} [tools]  tools allowlist; omitting it inherits the parent catalog
 */
...
/**
 * Run a vendor CLI to completion with the prompt on stdin, and hand back
 * everything it said.
 *
 * Every adapter under `lib/host/` needs exactly this and nothing more: a
 * subprocess, a prompt written to stdin (design D2 — never argv, so a briefing
 * with inlined rubrics cannot hit an argv limit and never appears in a process
 * listing), and the exit code plus both streams. Three copies of it would be
 * three chances for one adapter to start treating a signal, a timeout or a
 * non-zero exit differently from the others, and the difference would surface as
 * a run that reported a spawn as failed on one host and empty on another.
 *
 * Never rejects for anything the CLI did. A process that could not be started at
 * all resolves with `code: null` and the spawn error on stderr, because the
 * caller's contract is a `null` spawn result either way.
 *
 ...
 * @param {object} [opts.env]
 * @param {string} [opts.input]      the prompt, written to stdin and closed
 * @param {number} [opts.timeoutMs]  0 or absent disables the wall clock
 * @returns {Promise<{code: number|null, stdout: string, stderr: string, signal: string|null, timedOut: boolean}>}
 */
export function execCapture(command, args = [], { cwd, env, input, timeoutMs } = {}) {
  return new Promise(resolve => {
    let child
    try {
      child = spawnProcess(command, Array.isArray(args) ? args.map(String) : [], {
        cwd,
        env,
        stdio: ['pipe', 'pipe', 'pipe']
      })
    } catch (err) {
      resolve({ code: null, stdout: '', stderr: `${command}: ${(err && err.message) || err}\n`, signal: null, timedOut: false })
      return
    }
    ...
    child.on('error', err => {
      done({ code: null, stdout, stderr: `${stderr}${command}: ${(err && err.message) || err}\n`, signal: null, timedOut })
    })
    child.on('close', (code, signal) => {
      done({
        code: typeof code === 'number' ? code : null,
        stdout,
        stderr: timedOut ? `${stderr}${command}: killed after ${budget}ms\n` : stderr,
        signal: signal || null,
        timedOut
      })
    })

    // A stdin the child never reads closes with EPIPE, which is the child's
    // choice and not a transport failure.
