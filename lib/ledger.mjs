...
const EMPTY_MARKERS = new Set(['', '-', '--', '---', '—', '–', 'n/a', 'na', 'tbd', '?'])
...
/** True when a cell carries no value: blank, or one of the dash placeholders. */
...
export function summarize(parsed) {
  ...
  return {
    ...
    blocking: needsHuman > 0 || invalidCount > 0 || missingChange
  }
}
...
export function readLedger(root = '.', change) {
  ...
  if (!existsSync(path)) {
    return { exists: false, changeExists, path, change, rows: [], invalid: [] }
  }
...
export function formatLedger(parsed) {
  ...
  if (parsed && parsed.exists === false) {
    lines.push(`DECISIONS — none recorded (no ${LEDGER_FILE})`)
    return lines.join('\n') + '\n'
  }
  lines.push(
