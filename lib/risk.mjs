...
const DOC_PATH = /\.(md|mdx|txt|rst|adoc)$|(^|\/)(docs?|openspec|\.claude)\//
const TEST_PATH =
...
export function classifyRisk(input, opts = {}) {
  ...
  const docLike = paths.filter(p => DOC_PATH.test(p.lower) || TEST_PATH.test(p.lower))
  if (paths.length > 0 && docLike.length === paths.length) {
    ...
    const sourcePaths = paths.filter(p => !DOC_PATH.test(p.lower) && !TEST_PATH.test(p.lower))
    signals.push({
