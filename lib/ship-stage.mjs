...
//
// WHO WRITES IT. Not a driver. `lib/prompts/stage.mjs` renders the publish
// fragment from THIS module's own `stagePath`, `MARKER_FIELDS` and `STAGES`, and
// the agent performing each step writes the file as it enters the stage — so the
// path and JSON shape have exactly one definition, here, and no driver carries a
// second copy of them. `test/spine/ship-stage-drift.test.mjs` asserts that: it
// fails if any driver declares a marker literal at all.
//
// WHO CLEARS IT. `runClose` in `lib/run.mjs`, on every terminal path — the
// program routes a halt through `run close --halt` and both drivers' `stop()`
// call it, so the clean close and the halt clear the same way. A clear that
// fails is spoken as a degradation, never as an exit code: the marker is a guard
// input, not a gate.
 ...
 * `root` (default cwd). The only derivation of it: `lib/prompts/stage.mjs`
 * renders the agent's publish fragment from this function, and the drift test
 * fails if a driver ever declares the path as a literal of its own.
 */
