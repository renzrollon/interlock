...
// call.
//
// Exposed to skills as `interlock surface --changed <files...>`.
//
...
//
// Returns {
//   files: [{ path, class, reason, infra }],  // normalized path, one entry per usable input
//                                       // `infra` is this file's own infra-impact
//                                       // verdict — the per-file half of the
//                                       // needsDevopsReview rollup, carried so a
//                                       // caller naming the triggering paths does
//                                       // not re-run the classifier per path
...
export function classifySurface(changedFiles) {
  ...
  for (const entry of list) {
    ...
    const infra = suggestsInfraImpact(p, record)
    files.push({ path, class: record.class, reason: record.reason, infra })
    if (record.class === UI_TESTABLE) uiCount += 1
    else if (record.class === UI_INDIRECT) indirectCount += 1
    else backendCount += 1
    if (infra) needsDevopsReview = true
  }
