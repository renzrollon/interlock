...
function rows(list, { whole }) {
  ...
  if (all.length <= cap) return { shown: all, omitted: 0, note: null }
  return {
...
export function formatResumeCard({
  ...
  if (!leftover.shown.length) {
    ...
    lines.push('')
    lines.push(
      'Some of these may already be done on disk and only unticked. Tick those before ' +
        're-shipping, or they are implemented a second time:'
    )
    lines.push('')
    lines.push('```bash')
    lines.push(`interlock tasks tick ${name} --ids ${leftover.shown.join(',')}`)
    lines.push('```')
  }
  lines.push('')
