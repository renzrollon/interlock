 ...
 *
 * Every `kind` the run program spawns must be listed here: the field mapper
 * coerces an unlisted value to `other` WITHOUT a warning, so a kind missing from
 * this list reads back as an indistinguishable `other` agent and any later
 * reader that segments spend or failure by kind cannot tell a remediation round
 * from a commit. `test/spine/run-log.test.mjs` walks the `kind:` literals in
 * `lib/run.mjs` against this list for exactly that reason.
 */
...
// Bounds for the two receipt path sets. Deliberately here rather than in
// `lib/limits.mjs`: that module publishes *policy* caps through
// `interlock limits`, and nothing halts on either of these. They bound what one
// receipt writes, exactly as `MAX_TEXT` and the `spill` bound already do, and a
// set that reaches the bound is recorded as truncated rather than silently
// shortened (see `pathSetTruncated`).
const MAX_PATH_SET = 200
const MAX_PATH = 300
...
function waveSpend(value) {
  ...
  return value.filter(isObject).map(entry => ({
    ...
    cacheReadInputTokens: nullableCount(entry.cacheReadInputTokens),
    cacheCreationInputTokens: cacheTiers(entry.cacheCreationInputTokens)
  }))
}

// How many lifetime tiers one record may carry. A bound, not a policy cap: the
// host reports two today, and the tier names are read off the source rather
// than enumerated here, so this is what keeps a malformed payload from writing
// an unbounded object into a file agents read back.
const MAX_CACHE_TIERS = 8

/**
 * The per-lifetime-tier cache-creation block, bounded and tri-state.
 *
 * The tier NAMES come from the host, so they are validated by shape rather than
 * against a list this module would have to keep in step with an adapter. A
 * malformed key is dropped; a malformed figure is `null`, never `0`.
 *
 * `null` for a record that carried no decomposition at all — a host with no
 * cache accounting — which is a different fact from `{}`. Never summed into one
 * total: the two tiers are priced differently and the flattening is
 * irreversible (design D7).
 */
function cacheTiers(value) {
  if (!isObject(value)) return null
  const out = {}
  for (const [tier, figure] of Object.entries(value)) {
    if (!/^[a-z][a-z0-9_]{0,39}$/.test(tier)) continue
    out[tier] = nullableCount(figure)
    if (Object.keys(out).length >= MAX_CACHE_TIERS) break
  }
  return out
}

/**
 * The receipt's host block, bounded and tri-state throughout.
 *
 * Absent for a run whose close never named a host, which is a different fact
 * from a host that declared it has no hooks: the first is a receipt this reader
 * cannot compare, the second is a run that was deliberately weaker. Flooring
 * either to `false` would file them as the same run.
 */
function hostBlock(value) {
  if (!isObject(value)) return null
  return {
    id: nullableText(value.id, MAX_NAME),
    billing: nullableText(value.billing, MAX_NAME),
    hooks: nullableBool(value.hooks),
    usage: nullableBool(value.usage),
    // Declared separately from `usage` and recorded separately, so a reader can
    // partition a corpus by whether cache figures were obtainable at all rather
    // than averaging a measured figure against an absent one.
    cacheAccounting: nullableBool(value.cacheAccounting)
  }
}
...
// Tri-state array, and the reason `stringArray` could not be reused for the two
// receipt path sets: it floors an absent source to `[]`, which would file "this
// run made no commit" as "a commit touched nothing". That is the same collapse
// `nullableCount` and `nullableBool` exist to prevent one type down, and here it
// would be the denominator of an indicator — a measured zero invented out of an
// absent measurement. `null` means unobserved; `[]` means observed and empty.
//
// Bounded on both axes: at most `max` entries, each at most `maxLength`
// characters. A source past the bound is cut here and marked truncated by
// `pathSetTruncated` from the same source, so the two can never disagree.
function nullableStringArray(value, max = MAX_PATH_SET, maxLength = MAX_PATH) {
...
// Whether a path set reached its bound, derived from the same source the set is
// derived from rather than reported by the caller. A truncated set is not the
// set the run touched or predicted, and a reader comparing over it would be
// comparing over a denominator nobody stated. `null` when the set itself is
// unobserved: an unread set was not truncated, it was never read.
function pathSetTruncated(value, max = MAX_PATH_SET) {
  if (!Array.isArray(value)) return null
  return value.filter(v => typeof v === 'string' && v.trim()).length > max
}
...
const TYPE_FIELDS = {
  'run-start': {
    ...
    // The host session this run executed under, so a trajectory joins to the
    // host's own transcript by a key rather than by overlapping wall clocks.
    // Timestamp overlap is not an identity: two sessions in one project produce
    // overlapping windows that cannot be told apart without reading them.
    //
    // `nullableText`, never a synthesised substitute. A host that exposes no
    // session id records absent; deriving one from the runId, the pid or the
    // timestamp would put a key in the corpus that joins to nothing and looks
    // exactly like one that joins to something.
    sessionId: source => nullableText(source.sessionId, MAX_NAME)
  },
  'wave-action': {
  ...
  'run-receipt': {
    // Which host ran this and what it could not do. Copied by name and bounded
    // like everything else: the id is the runner's adapter (or the Workflow
    // host), and the three flags are the run's declared billing path, whether
    // this repository's guards were in force, and whether the host reported any
    // token usage at all. Absent on a receipt written before hosts declared
    // capabilities, which reads as unknown rather than as the Workflow host.
    host: source => hostBlock(source.host),
    ...
    // The input side of the same span, under the same unknown-not-zero rule.
    // Cache creation stays split by the lifetime tier the host reported it
    // under: a prefix written for five minutes and one written for an hour are
    // priced differently, so a single total cannot be priced and no reader
    // could split it back apart.
    cacheReadInputTokens: source => nullableCount(source.cacheReadInputTokens),
    cacheCreationInputTokens: source => cacheTiers(source.cacheCreationInputTokens),
    ...
    // The two path sets `interlock report` compares to answer "did the merged
    // diff match the plan". Copied by name and bounded like everything else
    // here, which matters more than usual: the source object these come off
    // carries the commit result and the executed plan, so a pass-through would
    // put diff hunks and file contents in a file future agents read back.
    //
    // The touched set is read from version control, never from the commit
    // agent's own account of what it committed — an agent that mis-listed its
    // own commit would silently move the indicator's denominator.
    touchedPaths: source => nullableStringArray(source.touchedPaths),
    touchedPathsTruncated: source => pathSetTruncated(source.touchedPaths),
    touchedPathsReason: source => nullableText(source.touchedPathsReason),
    // Path prediction is optional per task by design (`lib/waves.mjs`: a
    // classifier that cannot predict which files a task will touch must be able
    // to say nothing rather than guess), so the set alone cannot tell a plan
    // that predicted nothing extra from a plan that declined to predict. The
    // completeness marker carries that difference; the report excludes an
    // incomplete prediction rather than counting it as a run that went off-plan.
    predictedPaths: source => nullableStringArray(source.predictedPaths),
    predictedPathsTruncated: source => pathSetTruncated(source.predictedPaths),
    predictedPathsComplete: source => nullableBool(source.predictedPathsComplete),
    predictedPathsReason: source => nullableText(source.predictedPathsReason),
...
// Did either recorded path set reach its bound? Tri-state: `true` if either
// says so, `false` if both were observed untruncated, `null` if neither was
// observed — an unread set was not untruncated, it was never read.
function eitherTruncated(receipt) {
  const a = nullableBool(receipt.touchedPathsTruncated)
  const b = nullableBool(receipt.predictedPathsTruncated)
...
function summarizeRunLog(root, runId) {
  ...
  return {
    ...
    // Counts rather than the sets themselves — a list summary should not carry
    // four hundred path strings — but tri-state like `committed`, because the
    // whole value of these two fields is that "the run never found out" stays
    // apart from "the run measured nothing". A count of 0 is an observed empty
    // set; `null` is an unobserved one, and the reason says which.
    touchedPathCount: receipt && Array.isArray(receipt.touchedPaths) ? receipt.touchedPaths.length : null,
    touchedPathsReason: receipt ? nullableText(receipt.touchedPathsReason) : null,
    predictedPathCount: receipt && Array.isArray(receipt.predictedPaths) ? receipt.predictedPaths.length : null,
    predictedPathsReason: receipt ? nullableText(receipt.predictedPathsReason) : null,
    predictedPathsComplete: receipt ? nullableBool(receipt.predictedPathsComplete) : null,
    pathsTruncated: receipt ? eitherTruncated(receipt) : null,
...
// One path set, rendered so an unobserved set can never be read as an empty
// one. `unknown` carries the recorded reason; an observed empty set says out
// loud that it was observed. Neither renders as blank and neither renders
// as `[]`, which a reader would take for a measured zero.
function pathSetLine(label, set, reason, truncated) {
  if (!Array.isArray(set)) {
    return `${label}: unknown (${text(reason) || 'no reason recorded'})`
  }
  const mark = truncated === true ? ', TRUNCATED at the recorded bound — a prefix, not the whole set' : ''
  if (!set.length) return `${label}: none — observed and empty${mark}`
  return `${label}: ${set.length} path(s)${mark}: ${set.join(', ')}`
}
...
function receiptLines(r) {
  ...
  out.push(pathSetLine('paths touched (read from version control)', r.touchedPaths, r.touchedPathsReason, r.touchedPathsTruncated))
  out.push(pathSetLine('paths predicted (from the executed plan)', r.predictedPaths, r.predictedPathsReason, r.predictedPathsTruncated))
  out.push(
    `  prediction complete: ${
      r.predictedPathsComplete === true
        ? 'yes — every executed task declared its paths'
        : r.predictedPathsComplete === false
          ? 'no — at least one executed task declared none'
          : 'unknown'
    }`
  )
