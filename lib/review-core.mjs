...
// containing spaces — that case is handled by diff membership above.
const CITATION_TOKEN = /(\S+?):(\d+)(?:-\d+)?(?::\d+)?(?![\w.\-/])/g

// Does `text` cite `path` with a line number after it?
function citesLineIn(text, path) {
  for (let from = 0; ; ) {
...
// The reviewed diff as canonical identities plus the spellings the caller used,
// so a citation can be matched either way.
function diffIndex(changedPaths) {
 ...
 * @param {{band?: object|null, changedPaths?: string[]}} [opts] `band` overrides
 *   the tolerance band (`null` disables it — the default is `TOLERANCE_BAND`).
 *   `changedPaths` is the reviewed diff; when supplied, a dismissing verdict's
 *   citation must name a path in it (see `hasEvidence`).
 * @returns {{
