...
import { LIMITS, LANE_CAPS, SOLO } from './limits.mjs'
 ...
 * whose batches held tasks; `/2` is the plan whose batches hold lanes; `/3` is
 * the plan that carries a mode and whose lanes may be cohesion folds. A stored
 * plan carrying another version is not reused however well its inputs match.
 *
 * The bump is what forces a `/2` plan to be rebuilt rather than reused. A `/2`
 * plan's inputs can match perfectly and it will still hold one agent per
 * disjoint task and no mode at all — reusing it would silently keep the old
 * shape while the run believed it had the new one.
 */
export const PLAN_FORMAT = 'interlock.ship-plan/3'
...
/**
 * The dependency edge set a plan was built over, as one canonical line.
 *
 * `dependsOn` is a structural ordering signal that the artifact digests cannot
 * see: two classifications of one unedited `tasks.md` can differ only in their
 * edges, and the resulting plans schedule different work in different batches.
 * So a plan carrying edges is a different plan, and the fingerprint says so.
 *
 * Sorted, and empty edges are dropped, so the line is a pure function of the
 * edge set rather than of the order the classifier happened to emit. A task
 * list with no edges anywhere contributes NOTHING — not an empty line — which
 * is what keeps a fingerprint over an edge-free change byte-identical to the
 * one this module computed before edges existed.
 *
 * @param {unknown} tasks a classified task list, or `{tasks: [...]}`
 * @returns {string|null} the canonical line, or null when there are no edges
 */
function edgesOf(tasks) {
  ...
  for (const t of list) {
    ...
    const deps = [
      ...new Set(t.dependsOn.filter(d => typeof d === 'string' && d.trim()).map(d => d.trim()))
    ].sort()
    if (deps.length) edges.push(`${t.id}<-${deps.join(',')}`)
  }
  if (!edges.length) return null
  return `depends-on ${edges.sort().join(' ')}`
}

/**
 * Every cap and switch a plan's SHAPE depends on, as opposed to its content.
 *
 * `mode` is the override in force, not the mode the plan came out as: the
 * classifier's recommendation is an OUTPUT of planning, so hashing it would make
 * the fingerprint depend on something no reuse check can recompute without
 * running the classifier — which is the cost reuse exists to avoid. `auto`
 * records "no override was passed", and it is a distinct value from both `solo`
 * and `waves` because a plan built under `--waves` and one built under no flag
 * differ the moment a classifier starts recommending solo.
 */
function capsOf(opts = {}) {
  const requested = opts.maxParallel
  const mode = opts.mode === 'solo' || opts.mode === 'waves' ? opts.mode : 'auto'
  // The red wave a plan was built under. `null` is "no TDD shape", which is
  // what every plan before this existed was built under — see the canonical
  // text below for why that case must not appear in the hash at all.
  const redWave = Number.isInteger(opts.redWave) ? opts.redWave : null
  return {
    maxParallel:
      Number.isInteger(requested) && requested > 0 ? requested : LIMITS.maxParallel,
    mode,
    redWave,
    laneCaps: { ...LANE_CAPS.byTier },
    cohesionMaxTier: LANE_CAPS.cohesionMaxTier,
    soloMaxTasks: SOLO.maxTasks
  }
}

/** `1:8 2:8 3:6 4:4 5:8` — the tier table as one stable, sorted line. */
function laneCapsLine(table) {
  return Object.keys(table)
    .map(Number)
    .sort((a, b) => a - b)
    .map(tier => `${tier}:${table[tier]}`)
    .join(' ')
}
...
export function computeFingerprint(root = '.', change, opts = {}) {
  ...
  try {
    ...
    // Appended after the caps and before the digests, and OMITTED ENTIRELY when
    // the task list carries no edges — a null here has to leave the canonical
    // text exactly as it was, or every existing fingerprint would invalidate
    // for a signal none of them use.
    const edges = edgesOf(opts.tasks)
    const canonical = [
      `plan-format ${PLAN_FORMAT}`,
      `change ${base.change}`,
      `max-parallel ${caps.maxParallel}`,
      `mode ${caps.mode}`,
      `lane-caps ${laneCapsLine(caps.laneCaps)}`,
      `cohesion-max-tier ${caps.cohesionMaxTier}`,
      `solo-max-tasks ${caps.soloMaxTasks}`,
      // OMITTED ENTIRELY when there is no red wave, for the same reason `edges`
      // is: a plan built without a TDD shape must hash exactly as it did before
      // this line existed, or every stored fingerprint in every consumer repo
      // invalidates at once for a signal none of them use. Present, it MUST be
      // hashed — a cached non-TDD plan reused for a --tdd run would run the
      // failing suite last while the run believed it had the new shape.
      ...(caps.redWave !== null ? [`red-wave ${caps.redWave}`] : []),
      ...(edges ? [edges] : []),
      ...inputs.map(i => `${i.path} ${i.sha256 === null ? 'ABSENT' : i.sha256}`)
    ].join('\n')

    const problems = inputs.filter(i => i.sha256 === null).map(i => `unreadable: ${i.path}`)
    // Carried on the result as well as hashed into it, so a reader can see WHY
    // two fingerprints over identical artifacts differ.
    return { ...base, ok: true, hash: sha256(canonical), edges, inputs, problems }
  } catch (err) {
    return {
      ...base,
      ok: false,
...
export function formatFingerprint(fp) {
  ...
  lines.push(
    `  maxParallel ${fp.maxParallel}, mode ${fp.mode}` +
      (fp.redWave != null ? `, red wave ${fp.redWave}` : '')
  )
  lines.push(
    `  lane-caps ${laneCapsLine(fp.laneCaps || LANE_CAPS.byTier)}, ` +
      `cohesion-max-tier ${fp.cohesionMaxTier}, solo-max-tasks ${fp.soloMaxTasks}`
  )
  if (fp.edges) lines.push(`  ${fp.edges}`)
