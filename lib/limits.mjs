...
export const LIMITS = {
  ...
  /**
   * Steps one run of the ship program may take before it halts. A step is one
   * CLI call that yields a step record; a run that has taken this many without
   * reaching a terminal state is not converging.
   *
   * This was `MAX_LOOP_STEPS`, a literal restated in both drivers — a loop bound
   * the cap-authority spec forbids stating anywhere but here. It is enforced by
   * `lib/run.mjs`, which counts on the run manifest and returns a `halt` step
   * naming this cap once it is exceeded. A driver's own `RUNAWAY_BACKSTOP` is a
   * different thing: the host runtime's agent ceiling, not a policy cap.
   */
  maxRunSteps: 200,
  ...
  /**
   * Milliseconds a close's push to the ntfy relay may take before it is
   * abandoned as a failed push. A hanging network call must not stall the
   * close itself (design D5, add-harden-unattended-ship-runs). Read by
   * `postNtfy` in `lib/notify.mjs` via `AbortController`.
   */
  notifyTimeoutMs: 5000,

  /**
   * Rows of any one list — leftover task ids, wave tallies, degradation banners
   * — the halt resume card prints before it says how many it left out and names
   * the command that prints them all. The card is a pointer a person reads in
   * one sitting, not a second copy of the trajectory, so it is bounded; but a
   * silent head would make it lie, so the truncation is always stated. Read by
   * `lib/resume-card.mjs`.
   */
  resumeCardListRows: 40
}
...
export const EVAL_CAPS = {
  ...
  /**
   * US dollars. Ceiling for the scheduled **outcome** eval — the one that ships
   * a committed fixture through the real loop and grades what is on disk
   * (`evals/ship/run.mjs`), not the transcript-grading case suite above.
   *
   * Its two readers exist on the day it lands, which is the whole reason it may
   * be published at all: `evals/ship/run.mjs` reads it from `interlock limits
   * --json` before it starts a fixture, and `.github/workflows/ship-outcome-
   * eval.yml` reads the same field to bound the job. The number appears nowhere
   * else — not in the workflow YAML, not in the eval's README.
   *
   * Enforced at fixture and arm boundaries only. It stops the runner from
   * STARTING the next arm; it never kills a run in flight, because a
   * half-killed run cannot be graded and would be recorded as a failure it did
   * not earn (design D6). A sweep that reaches it is reported partial.
   *
   * A first guess, to be tuned from the spend the first rows actually report —
   * the same posture as the two ceilings above.
   */
  shipEvalCostUsd: 20,
...
/**
 * The price table the outcome eval converts measured tokens into dollars with,
 * so `EVAL_CAPS.shipEvalCostUsd` can be a ceiling on money rather than on a
 * token count nobody can compare across models (design D6).
 *
 * A separate export from EVAL_CAPS for the reason EFFORT and LANE_CAPS are: the
 * cap groups hold scalars a code path obeys, and this is a table. It lives here
 * all the same, because a price written in the runner and echoed in a workflow
 * would drift exactly like a prose cap.
 *
 * `id` IS THE LOAD-BEARING FIELD. It travels on every recorded result row, so a
 * later price revision cannot silently reinterpret rows written under the old
 * one. Revising a price therefore means a NEW id, never an edit to a number
 * under the existing one — old rows keep meaning what they meant, and a reader
 * comparing across a revision can see that it happened.
 *
 * The figures are the published list prices for each tier, in US dollars per
 * million tokens, as of the date in the id. They are an input to a ceiling and
 * to a recorded estimate — never to a bill, and never to a gate.
 *
 * A model that is not in the table yields NO cost figure: the runner records the
 * spend as absent with that reason rather than pricing it at zero or guessing a
 * tier from the name. An unpriced run still runs; it just cannot be counted
 * against the ceiling, and the sweep says so.
 *
 * Read by `evals/ship/run.mjs` through `interlock limits --json`; printed by
 * `formatLimits()` below.
 */
export const MODEL_PRICES = {
  // A NEW id, not an edit under `anthropic-list-2026-09`. The base rates below
  // are unchanged, but the table now answers a question the old one could not —
  // what a cached prefix costs — and a row keyed to the old id was priced by a
  // table with no cache structure at all. The `b` suffix rather than a new month
  // because this is a revision of the same published list, not a later one.
  id: 'anthropic-list-2026-09b',
  perMillionTokens: {
  ...
  /**
   * What a cached prefix costs, as multipliers on the model's own `input` rate
   * — the same shape for every model, which is why they are multipliers rather
   * than a fourth and fifth column per model.
   *
   * `write` is keyed by LIFETIME TIER and is never flattened: a prefix written
   * for five minutes and one written for an hour are different prices, and a
   * total that added them could not be priced back apart. The keys are the tier
   * names the host's usage envelope reports them under, so a recorded figure
   * and its multiplier are looked up by the same string.
   *
   * A read is a tenth of base, which is the whole economic argument: a prefix
   * that survives to be read costs 10% of what re-sending it costs, and a write
   * that is never read cost 25% (or 100%) more than not caching at all.
   *
   * NO READER IN THIS CHANGE, deliberately (design D9): the figures are recorded
   * so a dollar number becomes computable, and whether to compute and present one
   * is a later decision. They live in the table rather than beside it so the two
   * cannot drift — a run priced against one id and multiplied by a differently
   * versioned constant is attributable to nothing.
   */
  cacheMultipliers: {
    write: { ephemeral_5m: 1.25, ephemeral_1h: 2 },
    read: 0.1
  }
}
...
export function formatLimits() {
  const rows = [
    ...
    ['run steps (per run)', LIMITS.maxRunSteps],
    ...
    ['push timeout (ms)', LIMITS.notifyTimeoutMs],
    ['resume card list rows (per list)', LIMITS.resumeCardListRows]
  ]
  const evalRows = [
    ['eval smoke cost ceiling (per PR)', `$${EVAL_CAPS.smokeCostUsd}`],
    ['eval full-run cost ceiling (scheduled)', `$${EVAL_CAPS.fullRunCostUsd}`],
    ['eval outcome-run cost ceiling (scheduled)', `$${EVAL_CAPS.shipEvalCostUsd}`],
  ...
  // The price table prints beside the ceiling it serves: an operator reading a
  // dollar cap needs to see which prices turned tokens into those dollars, and
  // the id is what a recorded row is keyed against.
  const priceRows = [
    ['price table id', MODEL_PRICES.id],
    ...Object.keys(MODEL_PRICES.perMillionTokens)
      .sort()
      .map(model => [
        `price: ${model} ($/Mtok)`,
        `in ${MODEL_PRICES.perMillionTokens[model].input}, out ${MODEL_PRICES.perMillionTokens[model].output}`
      ]),
    // The cache multipliers print beside the base rates for the same reason the
    // base rates print beside the ceiling: they are part of what turns a token
    // count into a dollar figure, and a multiplier nobody can see is a price
    // written in prose. One row per lifetime tier, because they are never summed.
    ...Object.keys(MODEL_PRICES.cacheMultipliers.write)
      .sort()
      .map(tier => [`cache write multiplier: ${tier} (x input)`, MODEL_PRICES.cacheMultipliers.write[tier]]),
    ['cache read multiplier (x input)', MODEL_PRICES.cacheMultipliers.read]
  ]
  ...
  const width = Math.max(
    ...[...rows, ...laneRows, ...evalRows, ...priceRows, ...reportRows, ...effortRows].map(
      ([label]) => label.length
    )
  )
