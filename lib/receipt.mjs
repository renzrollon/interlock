  ...
  return {
    ...
    host: host
      ? {
          ...
          hooks: host.hooks,
          usage: host.usage,
          // Recorded per run so a reader can PARTITION the corpus rather than
          // average across it: cache figures exist on some hosts and not others,
          // and that asymmetry is stated rather than engineered away.
          cacheAccounting: host.cacheAccounting
        }
      : undefined,
    ...
    spend: (Array.isArray(summary.spend) ? summary.spend : []).map(entry => ({
      ...
      // The input side, carried on exactly the terms the output side is: named
      // element by element, never spread, and absent where the host reported
      // nothing. `cacheCreationInputTokens` stays a per-tier object all the way
      // through — a prefix written for five minutes and one written for an hour
      // are different prices, and flattening them here could not be undone.
 ...
 *
 * `runId`, `root`, `projectSlug`, `push`, `unarchived` and `resumeCard` all
 * default so this stays callable exactly as it was before design D7/D8/D9 and
 * the halt resume card added them — the receipt module stays import-free either
 * way.
 */
export function formatRunSummary({
  ...
  // The resume card: the one file a halted run leaves for a reader who was not
  // watching. Printed only when one was actually written — a close that could
  // not write it says so in the degradation block instead, and a clean run has
  // nothing to resume.
  if (typeof resumeCard === 'string' && resumeCard.length > 0) {
    lines.push(`  resume card: ${resumeCard}`)
  }
