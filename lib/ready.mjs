...
function readArtifactsFromDisk(inspection) {
  const out = {}
...
/**
 * Interpret the artifact-review result the caller passed.
 *
 * This is the gate's softest spot, so it is strict: only an object carrying a
 * usable `blockers` count counts as "the review ran". `undefined`, `true`, an
 * empty object and a negative count are all *unrun*, never zero.
 */
...
function extractScenarios(artifacts) {
  const found = []
...
function checkReview(run, review) {
  const r = readReview(review)
  if (!r.ran) {
    ...
    block(
      ...
      'REVIEW_NOT_RUN',
      `${r.reason} — an artifact review that did not run is not an artifact review with zero blockers`,
      []
    )
    return
  }
 ...
 * }} [opts]
 *   `review` is the artifact-review result — **omitting it blocks**, it is not
 *   read as zero blockers. `changedPaths`/`plannedPaths` feed `classifyRisk`.
...
export function assessReadiness(root, change, opts = {}) {
  ...
  checkReview(run, options.review)
