...
The report SHALL report the distribution of recorded plan-reuse statuses, and the share of runs in which the plan was revised mid-run. It SHALL additionally report whether the merged diff matched the plan as **not computable**, with the reason and with what recording it would require. It SHALL NOT present either recorded figure under that name.

#### Scenario: The diff-matches-plan indicator is a declared gap

- **WHEN** the report computes plan fidelity
- **THEN** the diff-matches-plan indicator is marked not computable, carries a reason, and carries what would be required to compute it
