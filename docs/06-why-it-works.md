...
**What this does not do:** `paths` is a model's prediction, so a task editing a file it never named is still unguarded by the collision check alone. That check narrows the race; it does not close it. The value is that the assumption is now *stated and checked* rather than assumed — an unpredicted collision is a wrong prediction, which is a thing that can be improved, rather than a silent property of the design.

Under `--isolate-waves` the race closes anyway, without needing the prediction to be right. Each lane in a batch runs in its own git worktree (`isolation: 'worktree'` on the lane's `agent()` spawn), so two lanes placed in one batch because their *predicted* paths were disjoint cannot overwrite each other even if the prediction was wrong — they never share a tree during the batch. After the batch's lanes return, `interlock merge-lanes` (backed by the pure `lib/merge-lanes.mjs`) folds each lane's worktree back into the shared tree: disjoint *actual* writes fold cleanly, and a real collision — two lanes that in fact *mutated* the same canonical path — halts the run, naming the contended path and the two lane labels, rather than resolving it with last-writer-wins. Mutated, not merely wrote: a lane that renames `lib/a.mjs` away deletes that source, so it contends with a lane that edited `lib/a.mjs`, even though neither lane's diff names the same destination. A copy's source is not a mutation and is deliberately left out, so a genuinely disjoint batch is not halted for it. A halt at merge time is the residual the prediction leaves open, surfaced loudly instead of silently discarded. `ship.js` derives each lane's worktree location from its stable `laneLabel`, so a resumed run locates the same worktree it wrote; a clean fold removes the batch's worktrees, a halt leaves the contenders on disk and names their locations. The flag is opt-in and off by default — a worktree spawn costs ~200–500 ms and disk per lane, and unset the batch runs exactly as described above.
